...
`npm run build:search-index` regenerates `dist/_data/searchindex.json` from the source catalog without running a full Astro build. `npm run editor` starts the local catalog editor shell, which can browse current catalog data, edit existing products, create new products with allocated SKUs, preview product Markdown, run validation, rebuild the search index, preview Git diffs, commit validated changes, and push.
...
- `npm run test:editor` verifies editor catalog APIs, Markdown preview, Git workflow status/diff safeguards, invalid-save handling, product save writes, next-SKU allocation, invalid department handling, product creation writes, and manifest version bump behavior.
