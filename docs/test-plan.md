...
| TST-016 | Local editor smoke test | Manual | `npm run editor` | Editor starts locally, loads products, filters products, edits existing product fields, allocates a new product SKU, previews rendered Markdown, runs validation, and runs the search-index rebuild action. |
| TST-017 | Local editor API write path | Automated | `npm run test:editor` | Editor APIs serve catalog/product data, render Markdown previews, reject invalid product saves, save an existing product, allocate next SKUs, reject unknown departments and out-of-sequence creates, create a new product, bump the catalog manifest version, and restore fixture files after the test. |
